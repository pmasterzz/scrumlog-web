...
//            $scrumlog[0]['Radio_Help'] = GetTeacherNameById($scrumlog[0]['Radio_Help']);
    ...
    $scrumlog = getScrumlog($date, $year, $student_ID, $seating, $cycle_ID);
    $response = $app->response();
    $response['Content-Type'] = 'application/json';
...
$app->get('/api/getLatestScrumlogs', function () use ($app) {
    $id = $app->request->params('id');
    $scrumlogs = getLatestScrumlogs($id);
    $response = $app->response();
    $response['Content-Type'] = 'application/json';
    $response->body(json_encode($scrumlogs));
    return $response;
});
$app->run();
